...
router.route('/').get(function (req, res) {
    ...
    db.serialize(() => {
        db.all("SELECT * FROM SEAT AS S JOIN FLIGHT AS F ON S.flight_id = F.flight_id WHERE F.flight_id = ? ", userData.optionsBack, function (err, rows) {
            // console.log(rows)
...
router.route('/add').get(function (req, res) {
    ...
    if (selectedSeats.length == passengersNo) {
        ...
        for (let i of selectedSeats) {
            db.serialize(() => {
                db.run("UPDATE SEAT SET occupied = 1 WHERE flight_id = ? AND code = ? ", [userData.optionsBack, i], function (err, rows) {
                    if (err) {
